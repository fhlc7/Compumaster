...
public class FrmParcela extends JFrame {
	...
	public FrmParcela() {
		...
		btnGerarParcelas.setMnemonic('p');
		...
		JButton btnGerarCarn = new JButton("Gerar Carn\u00EA");
		btnGerarCarn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				gerarCarne();
			}
		});
		btnGerarCarn.setMnemonic('c');
		panel_3.add(btnGerarCarn);
			...
			gl_panelParcela.createParallelGroup(Alignment.TRAILING)
				.addGroup(Alignment.LEADING, gl_panelParcela.createSequentialGroup()
					...
					.addGroup(gl_panelParcela.createParallelGroup(Alignment.LEADING)
						...
						.addComponent(panel_3, GroupLayout.DEFAULT_SIZE, 972, Short.MAX_VALUE)
	...
	private void gerarCarne() {
		...
		String valorParcela = JOptionPane.showInputDialog("Digite o valor da parcela:");
		int numeroParcela = 0;
		SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
		for (Parcela parcela : ParcelaControle.listar(aluno, turma)) {
			Carne carne = new Carne(
					aluno.getNomeAluno(), 
					String.valueOf(aluno.getId()),/*String.valueOf(parcela.getMatricula().getId()),*/
					String.valueOf(turma.getDiaDaSemana()), 
					String.valueOf(valorParcela == null ? "" : valorParcela), 
					String.valueOf(++numeroParcela),
					turma.getCurso(), 
					sdf.format(parcela.getDataDeVencimento().getTime()), 
					turma.getHorario());
			lista.add(carne);
		}
		ParcelaControle.gerarRelatorio(lista);
	}
}
